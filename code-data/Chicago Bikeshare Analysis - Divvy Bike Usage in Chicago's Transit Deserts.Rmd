...
divvy_comp %>%
  ...
  dplyr::summarise(MEAN.TRIP.NUM = mean(TRIP.NUM)) %>%
...
divvy_comp$USER.TYPE <- revalue(divvy_comp$USER.TYPE, replace = c("Customer" = "Single Riders", "Subscriber" = "Annual Member", "Dependent" = "24Hrs Unlimited Rides"))

divvy_comp %>%
  filter(!USER.TYPE == "24Hrs Unlimited Rides") %>%
  group_by(USER.TYPE) %>%
