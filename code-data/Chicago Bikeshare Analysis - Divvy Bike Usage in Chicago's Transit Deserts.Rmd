...
station_dates <- 
  ...
  select(STATION.ID, OPENING.DATE)

# Join opening date information to divvy_station dataframe
divvy_opening <- 
  divvy_stations %>%
  left_join(station_dates, by = c("ID" = "STATION.ID")) %>%
...
ggplot() +
  ...
  geom_point(data = divvy_stations2, aes(Longitude, Latitude, color = factor(OPENING.YEAR)), size = 0.3) +
...
```{r warning=FALSE, message=FALSE, error=FALSE}
divvy_comp %>%
  filter(year(START.TIME) == c(2014, 2015, 2016, 2017)) %>%
  filter(!is.na(Community.Areas)) %>%
  mutate(year = year(START.TIME), 
         month = month(START.TIME), 
         day = day(START.TIME), 
         wday(START.TIME, label = TRUE)) %>%
  group_by(year, month, day, Chicago.side) %>%
  count()
...
divvy_comp %>%
  ...
  summarise(MEAN.TRIP.NUM = mean(TRIP.NUM)) %>%
...
divvy_comp %>%
  ...
  summarise(median_time = median(TRIP.DURATION)/60) %>% # average trip by comm area in Minutes
  mutate(axis_color = ifelse(median_time > 11.5, "blue", "red")) %>%
  ggplot(aes()) +
  geom_segment(aes(x = reorder(as.factor(Community.Area.Name), median_time), xend = Community.Area.Name, y = 11.5, yend = median_time, color = axis_color), size=1.3, alpha=0.9) +
  theme(
...
## Plot 5.2: Proportion of population vs. proportion of trips originating in the area
```{r}
...
divvy_comp %>%
  ...
  ggplot(aes(x = 1, y = sum_rides, fill = Chicago.side)) +
  geom_col(position = "fill") +
  ggtitle("Proportion of rides per Chicago Side")

#Proportion of population per chicago side
divvy_comp %>%
  filter(!is.na(Community.Areas)) %>%
  filter(year(START.TIME) == c(2014, 2015, 2016, 2017)) %>%
  group_by(Community.Areas) %>%
  ...
  summarise(sum_pop = sum(pop), 
            sum_rides = sum(n_rides)) %>%
  ggplot(aes(x = 1, y = sum_pop, fill = Chicago.side)) +
  geom_col(position = "fill") +
  ggtitle("Proportion of population per Chicago side")
```

## Plot 5.2: Proportion of stations vs. proportion of trips originating in the area
```{r warning=FALSE, message=FALSE, error=FALSE}
divvy_comp %>%
  filter(!is.na(Community.Areas)) %>%
  filter(year(START.TIME) == 2017) %>%
  group_by(Chicago.side) %>%
  summarise(n_rides = n(),
            n_stations = n_distinct(FROM.STATION.NAME)) %>%
  gather(key = object, value = value, c(n_rides, n_stations)) %>%
